...
void socks_shell_cmd( string, srcshp, dstshp ) char *string;
...
static void do_child( daemon_pid, command ) int daemon_pid;
char *command;
{
   char *error = 0;
   int tmp_fd;
...
static void percent_x( result, result_len, str, srcshp, dstshp, pid ) char *result;
int result_len;
char *str;
struct sockshost_s *srcshp, *dstshp;
int pid;
...
ABCDEFGHIJKLMNOPQRSTUVWXYZ";
   ...
   while ( *str )
   {
      if ( *str == '%' )
      {
         str++;
         expansion =
            *str == 'A' ? ( str++, src_name ) : *str == 'a' ? ( str++, inet_ntoa( srcshp->shipaddr[0] ) )
                                             : *str == 'c'  ? ( str++, socks_cmd )
