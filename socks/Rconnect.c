...
static void do_C_proto( sock, port, addr ) int sock;
unsigned short port;
u_int32 addr;
{
   Socks_t dst;
   ...
   alarm( 0 );
   if ( ( dst.cmd == SOCKS_FAIL ) || ( dst.cmd == SOCKS_NO_IDENTD ) || ( dst.cmd == SOCKS_BAD_ID ) )
   {
      exit( dst.cmd );
   }
   exit( SOCKS_RESULT );
...
int size;
{
   ...
   if ( ( sock != socks_conn_sock ) || ( sin->sin_port != socks_conn_port ) || ( sin->sin_addr.s_addr != socks_conn_host ) )
   {
      if ( socks_conn_init )
      {
